...
if(($db['missao']>900) && ($db['missao']<=905)){
        ...
        if($agora>=$fim){
                ...
                if($db['missao_tempo'] >= 10 && isset($conexao)){
                    try {
                ...
                if(isset($conexao)) despertar_verificar_notificacao($conexao, $db['id']);
...
if($chance<=10){
        $sqli=mysql_query("SELECT * FROM table_itens ORDER BY RANDOM() LIMIT 1");
...
if($chance>=91){
        $sqli=mysql_query("SELECT * FROM table_usaveis ORDER BY RANDOM() LIMIT 1");
        $dbi=mysql_fetch_assoc($sqli);
