<?php
try {
    $sqlr = $conexao->prepare("SELECT * FROM inventario WHERE usuarioid=? ORDER BY RANDOM() LIMIT 3");
    $sqlr->execute([$db['id']]);
...
<div class="box_middle">Abaixo estão os itens comprados no Ichiraku Bar. Caso você tenha muitos itens em seu inventário, serão mostrados apenas 3, aleatóriamente. Para visualizar todos, utilize <a href="?p=inventory">esta</a> página.
    ...
    <?php } while($dbr=$sqlr->fetch(PDO::FETCH_ASSOC)); ?>
    <?php if($dbr && $sqlr->rowCount()>=3){ ?>
    <tr>
        <td colspan="2"><div class="sep"></div></td>
    </tr>
    <tr>
    ...
    <?php } ?>
    <?php } else { ?>
    <tr><td colspan="2"><div class="aviso">Nenhum item no inventário.</div></td></tr>
    <?php } ?>
    </table>
</div>
