...
try {
    $sqlp = $conexao->query("SELECT count(id) as conta FROM usuarios");
    $dbp = $sqlp->fetch();
...
if(isset($_POST['ram_id'])){
        ...
        vn($id); vn($qtdade);
        if(($id>5)or($id<=0)){ echo "<script>self.location='?p=ramen'</script>"; exit; }
        if(($qtdade>5)or($qtdade<=0)){ echo "<script>self.location='?p=ramen'</script>"; exit; }
        $total=$valor*$qtdade;
        if($db['yens']<$total){ echo "<script>self.location='?p=ramen&msg=1'</script>"; exit; }
        $stmt = $conexao->prepare("UPDATE usuarios SET yens=yens-? WHERE id=?");
        $stmt->execute([$total, $db['id']]);
        $i=0;
        do{
                $stmt = $conexao->prepare("INSERT INTO ramen (usuarioid, ramenid) VALUES (?, ?)");
                $stmt->execute([$db['id'], $id]);
                $i++;
        } while($i<$qtdade);
        echo "<script>alert('Ramen comprado com sucesso! Confira na aba 🍜 Ichiraku Ramen do seu inventário.');self.location='?p=home'</script>"; exit;
}
?>
