...
if(isset($_POST['inicial'])){
    ...
    if(($sexo<>'m')&&($sexo<>'f')&&($sexo<>'')){ echo "<script>self.location='?p=home'</script>"; exit; }
    ...
    try {
        $stmt = $conexao->prepare("UPDATE usuarios SET pessoal_nome=?, pessoal_sexo=?, pessoal_idade=?, pessoal_pais=?, pessoal_uf=?, config_apresentacao=?, config_pergunta=?, config_resposta=?, config_recuperacao=1 WHERE id=?");
        $stmt->execute([$nome, $sexo, $idade, $pais, $uf, $msg, $pergunta, $resposta, $db['id']]);
...
<fieldset><legend>Configurações Iniciais</legend>
    ...
    <span class="sub2">Email cadastrado (impossível alterar).</span><?php if(($db['config_recuperacao'] ?? 0)==0){ ?><br /><div class="sep"></div>
    ...
    <input type="text" id="inicial_resposta" name="inicial_resposta" value="<?php echo htmlspecialchars($db['config_resposta'] ?? ''); ?>" /><br />
...
<fieldset><legend>Texto de Apresentação</legend>
    <textarea id="inicial_msg" name="inicial_msg" style="width:100%;height:200px;"><?php echo htmlspecialchars($db['config_apresentacao'] ?? ''); ?></textarea>
    <span class="sub2">Texto que aparecerá na página do seu perfil.</span>
</fieldset>
<fieldset><legend>Dados Pessoais</legend>Preencha os dados abaixo para ajudar a <?php echo nome_servidor(); ?> a montar um quadro estatístico. Nenhum dos campos abaixo serão publicados na página de seu perfil.<div class="sep"></div>
    <span class="destaque">Nome Completo:</span><br />
    <input type="text" id="inicial_nome" name="inicial_nome" value="<?php echo htmlspecialchars($db['pessoal_nome'] ?? ''); ?>" /><br />
    ...
    <input type="text" id="inicial_idade" name="inicial_idade" size="5" maxlength="3" value="<?php echo htmlspecialchars($db['pessoal_idade'] ?? ''); ?>" /><br />
