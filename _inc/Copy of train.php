...
$src="_img/bars/nin.jpg";
$back="_img/bars/train_back.png";
...
<form method="post" action="?p=train">
...
<div class="box_middle">Esta é sua área de treino de atributos. Utilize as setas abaixo para aumentar ou diminuir os atributos, e assim que estiver satisfeito, clique no botão Treinar. Os yens só serão gastos após a confirmação do treino.<div class="sep"></div>
	...
	<div style="padding-left:5px;background:url(_img/gradient.jpg) repeat-y"><img src="_img/yens.png" width="14" height="14" align="absmiddle" /> <b>Meus Yens: <?php echo number_format($db['yens'],2,',','.'); ?> yens</b></div>
    <div class="sep"></div>
	<table width="100%" cellpadding="0" cellspacing="0">
  <tr>
        	<td width="13%" align="right" style="padding-right:10px;"><b>Taijutsu:</b></td>
          <td style="background:url(<?php echo $back; ?>) no-repeat center left;">
            <?php
			if($array[0]==$array2["t"]) echo '<img id="taibar" src="'.$src.'" width="'.($max*$array[0])/$array[0].'" height="3" style="padding-left:3px;">'; else
			if($array[1]==$array2["t"]) echo '<img id="taibar" src="'.$src.'" width="'.($max*$array[1])/$array[0].'" height="3" style="padding-left:3px;">'; else
			if($array[2]==$array2["t"]) echo '<img id="taibar" src="'.$src.'" width="'.($max*$array[2])/$array[0].'" height="3" style="padding-left:3px;">';
			?>
    		</td>
        ...
        <tr>
          ...
          <td style="background:url(<?php echo $back; ?>) no-repeat center left;">
            <?php
			if($array[0]==$array2["n"]) echo '<img id="ninbar" src="'.$src.'" width="'.($max*$array[0])/$array[0].'" height="3" style="padding-left:3px;">'; else
			if($array[1]==$array2["n"]) echo '<img id="ninbar" src="'.$src.'" width="'.($max*$array[1])/$array[0].'" height="3" style="padding-left:3px;">'; else
			if($array[2]==$array2["n"]) echo '<img id="ninbar" src="'.$src.'" width="'.($max*$array[2])/$array[0].'" height="3" style="padding-left:3px;">';
			?>
            </td>
        ...
        <tr>
          ...
          <td style="background:url(<?php echo $back; ?>) no-repeat center left;">
            <?php
			if($array[0]==$array2["g"]) echo '<img id="genbar" src="'.$src.'" width="'.($max*$array[0])/$array[0].'" height="3" style="padding-left:3px;">'; else
			if($array[1]==$array2["g"]) echo '<img id="genbar" src="'.$src.'" width="'.($max*$array[1])/$array[0].'" height="3" style="padding-left:3px;">'; else
			if($array[2]==$array2["g"]) echo '<img id="genbar" src="'.$src.'" width="'.($max*$array[2])/$array[0].'" height="3" style="padding-left:3px;">';
			?>
          </td>
  ...
  <div class="sep"></div>
    ...
    <div align="center"><input type="submit" class="botao" value="Treinar" /></div></div>
</div>
</form>
