...
try {
    $sqlo = $conexao->prepare("SELECT id,sigla,nome,nivel,logo,minimo FROM organizacoes WHERE servidor_id = ? ORDER BY nivel DESC");
    $sqlo->execute([(int)($_SESSION['servidor_id'] ?? 1)]);
    $dbo = $sqlo->fetch();
...
?>
    ...
    <div class="sep"></div>
    <?php if(!$dbo) echo '<div class="aviso">Nenhuma organização na sua vila até o momento.<br />Seja o primeiro a criar uma, clicando <a href="?p=createorg">aqui</a>!</div>'; else { ?>
    <table width="100%" cellpadding="0" cellspacing="1">
        ...
        <?php if($cor=='#323232') $cor='#2C2C2C'; else $cor='#323232'; } while($dbo=$sqlo->fetch()); ?>
    </table>
    <?php } ?>
</div>
