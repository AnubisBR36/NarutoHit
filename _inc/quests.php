...
if(isset($_GET['start'])){
        ...
        $stmtq = $conexao->prepare("SELECT * FROM table_quests WHERE id = ?");
        $stmtq->execute([$start_id]);
        $dbq = $stmtq->fetch(PDO::FETCH_ASSOC);
        if(!$dbq){ echo "<script>self.location='?p=home'</script>"; exit; }
        if($db['nivel']<$dbq['nivel']){ echo "<script>self.location='?p=quests&msg=1'</script>"; exit; }
        if($db['quest']>0){ echo "<script>self.location='?p=quests&msg=2'</script>"; exit; }
        $conexao->prepare("UPDATE usuarios SET quest = ?, quest_vitorias = 0 WHERE id = ?")->execute([$start_id, (int)$db['id']]);
        echo "<script>self.location='?p=quests&msg=3'</script>";
}
if(isset($_GET['receive'])){
        $stmtq = $conexao->prepare("SELECT * FROM table_quests WHERE id = ?");
        $stmtq->execute([(int)$db['quest']]);
        $dbq = $stmtq->fetch(PDO::FETCH_ASSOC);
        if(!$dbq){ echo "<script>self.location='?p=home'</script>"; exit; }
        if($db['quest_vitorias']<$dbq['vitorias']){ echo "<script>self.location='?p=quests&msg=5'</script>"; exit; }
        $conexao->prepare("UPDATE usuarios SET quest = 0, quest_vitorias = 0, yens = yens + ?, yens_fat = yens_fat + ?, exp = exp + ?, exptotal = exptotal + ? WHERE id = ?")
                ->execute([(int)$dbq['yens'], (int)$dbq['yens'], (int)$dbq['exp'], (int)$dbq['exp'], (int)$db['id']]);
        echo "<script>self.location='?p=quests&msg=6'</script>";
}
if(isset($_GET['cancel'])){
        $conexao->prepare("UPDATE usuarios SET quest = 0, quest_vitorias = 0 WHERE id = ?")->execute([(int)$db['id']]);
