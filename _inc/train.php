...
if(isset($_POST['train_taijutsu'])){
	if($_POST['restante']<0){ echo "<script>self.location='?p=train&msg=2'</script>"; exit; }
	$taijutsu=$_POST['train_taijutsu'];
	$ninjutsu=$_POST['train_ninjutsu'];
	$genjutsu=$_POST['train_genjutsu'];
	if(($taijutsu<=0)or($ninjutsu<=0)or($genjutsu<=0)){ echo "<script>self.location='?p=home'</script>"; exit; }
	$total=0;
	...
	if($total>$db['yens']){ echo "<script>self.location='?p=train&msg=2'</script>"; exit; }
	
	try {
		$stmt = $conexao->prepare("UPDATE usuarios SET yens=yens-?, taijutsu=?, ninjutsu=?, genjutsu=? WHERE id=?");
		$stmt->execute([$total, $taijutsu, $ninjutsu, $genjutsu, $db['id']]);
		echo "<script>self.location='?p=train&msg=1&yens=".$total."'</script>";
	} catch (PDOException $e) {
		echo "<script>self.location='?p=train&msg=2'</script>";
	}
}
?>
<script>
...
function atualizabarras(){
	...
	if(array[0]==array2[0]) {
		document.getElementById('taibar').setAttribute('width',Math.round((max*array[0])/array[0]));
		document.getElementById('taibar').setAttribute('src','_img/NewsBar/Azul/barra_centro.jpg');
	} else
	if(array[1]==array2[0]) {
		document.getElementById('taibar').setAttribute('width',Math.round((max*array[1])/array[0]));
		document.getElementById('taibar').setAttribute('src','_img/NewsBar/Azul/barra_centro.jpg');
	} else
	if(array[2]==array2[0]) {
		document.getElementById('taibar').setAttribute('width',Math.round((max*array[2])/array[0]));
		document.getElementById('taibar').setAttribute('src','_img/NewsBar/Azul/barra_centro.jpg');
	}
	if(array[0]==array2[1]) {
		document.getElementById('ninbar').setAttribute('width',Math.round((max*array[0])/array[0]));
		document.getElementById('ninbar').setAttribute('src','_img/NewsBar/Roxo/barra_centro.jpg');
	} else
	if(array[1]==array2[1]) {
		document.getElementById('ninbar').setAttribute('width',Math.round((max*array[1])/array[0]));
		document.getElementById('ninbar').setAttribute('src','_img/NewsBar/Roxo/barra_centro.jpg');
	} else
	if(array[2]==array2[1]) {
		document.getElementById('ninbar').setAttribute('width',Math.round((max*array[2])/array[0]));
		document.getElementById('ninbar').setAttribute('src','_img/NewsBar/Roxo/barra_centro.jpg');
	}
	if(array[0]==array2[2]) {
		document.getElementById('genbar').setAttribute('width',Math.round((max*array[0])/array[0]));
		document.getElementById('genbar').setAttribute('src','_img/NewsBar/Verde/barra_centro.jpg');
	} else
	if(array[1]==array2[2]) {
		document.getElementById('genbar').setAttribute('width',Math.round((max*array[1])/array[0]));
		document.getElementById('genbar').setAttribute('src','_img/NewsBar/Verde/barra_centro.jpg');
	} else
	if(array[2]==array2[2]) {
		document.getElementById('genbar').setAttribute('width',Math.round((max*array[2])/array[0]));
		document.getElementById('genbar').setAttribute('src','_img/NewsBar/Verde/barra_centro.jpg');
	}
}
function change(at,dir){
	...
	else if((taijutsu>taipadrao)||(ninjutsu>ninpadrao)||(genjutsu>genpadrao))
		document.getElementById('train_button').style.display='block';
	atualizabarras();
}
</script>
<?php
...
<form method="post" action="?p=train" onsubmit="document.getElementById('subm').value='Carregando...';document.getElementById('subm').disabled=true;">
...
<div class="train_stats_bg">
  ...
  <tr class="attribute-row">
        	<td width="13%" align="right" style="padding-right:10px;background-color:#1b1b1a;"><b>Taijutsu:</b></td>
          <td><img src="_img/NewsBar/Azul/ponta_barra.jpg" height="22" /><?php
			if($array[0]==$array2["t"]) echo '<img id="taibar" src="_img/NewsBar/Azul/barra_centro.jpg" width="'.($max*$array[0])/$array[0].'" height="22" />'; else
			if($array[1]==$array2["t"]) echo '<img id="taibar" src="_img/NewsBar/Azul/barra_centro.jpg" width="'.($max*$array[1])/$array[0].'" height="22" />'; else
			if($array[2]==$array2["t"]) echo '<img id="taibar" src="_img/NewsBar/Azul/barra_centro.jpg" width="'.($max*$array[2])/$array[0].'" height="22" />';
        ...
        <tr class="attribute-row">
          ...
          <td><img src="_img/NewsBar/Roxo/ponta_barra.jpg" height="22" /><?php
			if($array[0]==$array2["n"]) echo '<img id="ninbar" src="_img/NewsBar/Roxo/barra_centro.jpg" width="'.($max*$array[0])/$array[0].'" height="22" />'; else
			if($array[1]==$array2["n"]) echo '<img id="ninbar" src="_img/NewsBar/Roxo/barra_centro.jpg" width="'.($max*$array[1])/$array[0].'" height="22" />'; else
			if($array[2]==$array2["n"]) echo '<img id="ninbar" src="_img/NewsBar/Roxo/barra_centro.jpg" width="'.($max*$array[2])/$array[0].'" height="22" />';
        ...
        <tr class="attribute-row">
          ...
          <td><img src="_img/NewsBar/Verde/ponta_barra.jpg" height="22" /><?php
			if($array[0]==$array2["g"]) echo '<img id="genbar" src="_img/NewsBar/Verde/barra_centro.jpg" width="'.($max*$array[0])/$array[0].'" height="22" />'; else
			if($array[1]==$array2["g"]) echo '<img id="genbar" src="_img/NewsBar/Verde/barra_centro.jpg" width="'.($max*$array[1])/$array[0].'" height="22" />'; else
			if($array[2]==$array2["g"]) echo '<img id="genbar" src="_img/NewsBar/Verde/barra_centro.jpg" width="'.($max*$array[2])/$array[0].'" height="22" />';
