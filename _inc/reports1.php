...
$min=($pg*10);
$sqlc=mysql_query("SELECT count(id) conta FROM relatorios WHERE usuarioid=".$db['id']);
if($sqlc && mysql_num_rows($sqlc) > 0) {
    $dbc=mysql_fetch_assoc($sqlc);
    $qt=$dbc['conta'];
} else {
    $qt = 0;
}
$sqlr=mysql_query("SELECT r.*, u.usuario usuario, u2.usuario inimigo FROM relatorios r LEFT OUTER JOIN usuarios u ON r.usuarioid=u.id LEFT OUTER JOIN usuarios u2 ON r.inimigoid=u2.id WHERE r.usuarioid=".$db['id']." ORDER BY id DESC LIMIT $min,10");
if($sqlr) {
