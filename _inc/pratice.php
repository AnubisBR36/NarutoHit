...
<?php
try {
    $jutsu_id = $c->decode($_GET['jutsu'], $chaveuniversal);
    $stmt = $conexao->prepare("SELECT id,nome,natureza,forca,nivel,valor FROM table_jutsus WHERE id=?");
    $stmt->execute([$jutsu_id]);
    $dbj = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if(!$dbj || $dbj['nivel'] > $db['nivel']){ 
        echo "<script>self.location='?p=learn&id=".$_GET['id']."'</script>"; 
        return; 
    }
    if($dbj['valor'] > $db['yens']){ 
        echo "<script>self.location='?p=learn&id=".$_GET['id']."&msg=2'</script>"; 
        return; 
    }
    if(($dbj['natureza'] != 'nenhum') && ($db['natureza1'] != $dbj['natureza']) && ($db['natureza2'] != $dbj['natureza']) && ($db['natureza3'] != $dbj['natureza'])){ 
        echo "<script>self.location='?p=learn&id=".$_GET['id']."'</script>"; 
        return; 
    }
} catch (PDOException $e) {
...
if(isset($_POST['contador'])){
        if($_POST['contador'] >= floor($dbj['forca']/4)){
                try {
                        $check_stmt = $conexao->prepare("SELECT count(id) as conta FROM jutsus WHERE usuarioid=? AND jutsu=?");
                        $check_stmt->execute([$db['id'], $dbj['id']]);
                        $dbv = $check_stmt->fetch(PDO::FETCH_ASSOC);
                        ...
                        if($dbv['conta'] == 0){
                                ...
                                $insert_jutsu = $conexao->prepare("INSERT INTO jutsus (usuarioid, jutsu, nivel, exp, expmax) VALUES (?, ?, 1, 0, 50)");
                                $insert_jutsu->execute([$db['id'], $dbj['id']]);
                                
                                $insert_update = $conexao->prepare("INSERT INTO atualizacoes (usuarioid, texto, hora) VALUES (?, ?, ?)");
                                $update_text = '<a href="?p=view&view='.strtolower($db['usuario']).'">'.$db['usuario'].'</a> aprendeu <b>'.$dbj['nome'].'</b>.';
                                $insert_update->execute([$db['id'], $update_text, time()]);
                                
                                $update_yens = $conexao->prepare("UPDATE usuarios SET yens=yens-? WHERE id=?");
                                $update_yens->execute([$dbj['valor'], $db['id']]);
                                
                                $conexao->commit();
                                
                                echo "<script>self.location='?p=room&id=".$_GET['id']."&msg=2'</script>"; 
                                return;
                        } else { 
                                echo "<script>self.location='?p=room&id=".$_GET['id']."'</script>"; 
                                return; 
                        }
                } catch (PDOException $e) {
                        $conexao->rollBack();
...
<div class="box_middle">É hora de treinar! Seja o mais rápido possível, e faça os selos que lhe mostrarei. Caso você erre, começará tudo novamente. Completando os <?php echo floor($dbj['forca']/4); ?> selos, você estará pronto para utilizar o jutsu!<div class="sep"></div>
    ...
    <script>
        ...
        function minigame(id){
                ...
                if(conta>=<?php echo floor($dbj['forca']/4); ?>){
    ...
    <table width="100%" cellpadding="0" cellspacing="1" style="background:url(_img/school/fundo_selos.jpg) center no-repeat;">
        <tr>
          ...
          <td align="center"><div id="conta" style="font-size:18px;font-weight:bold;margin-top:7px;margin-right:25px;color:#666666;">Acertos: <span id="count">0</span>/<?php echo floor($dbj['forca']/4); ?></div></td>
          </tr>
        <tr>
