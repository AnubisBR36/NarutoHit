...
if(isset($_POST['enq_id'])){
        $resp_valida = in_array($_POST['enq_resposta'] ?? '', ['a','b','c','d','e'], true);
        if($resp_valida && is_numeric($_POST['enq_id'])){
                $col = 'resp_' . $_POST['enq_resposta'];
                $conexao->prepare("UPDATE enquetes SET $col = $col + 1 WHERE id = ?")->execute([(int)$_POST['enq_id']]);
                $_SESSION['enq_'.(int)$_POST['enq_id']]=1;
        }
...
<div class="box_middle">Vote em nossas enquetes e ajude-nos a melhorar o jogo! As enquetes com o ícone azul ainda estão abertas para voto. As enquetes de ícone cinza já encerraram, exibindo os resultados finais.
        ...
        if(isset($_GET['msg'])){
                switch($_GET['msg']){
                        case 1: $msg='Voto computado com sucesso! A equipe '.nome_servidor().' agradece pelo seu voto.'; break;
