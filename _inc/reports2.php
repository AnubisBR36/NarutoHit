...
$min=($pg*10);
try {
    $sqlc = $conexao->prepare("SELECT count(id) as conta FROM relatorios WHERE inimigoid=?");
    $sqlc->execute([$db['id']]);
...
try {
    $sqlr = $conexao->prepare("SELECT r.*, u.usuario usuario, u2.usuario inimigo FROM relatorios r LEFT OUTER JOIN usuarios u ON r.usuarioid=u.id LEFT OUTER JOIN usuarios u2 ON r.inimigoid=u2.id WHERE r.inimigoid=? ORDER BY status DESC, id DESC LIMIT ?,10");
