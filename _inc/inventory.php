
<?php
require_once('trava.php');
require_once('Encrypt.php');
$c=new C_Encrypt();

if(isset($_GET['action'])){
        if(!isset($_GET['id'])){ echo "<script>self.location='?p=home'</script>"; exit(); }
        $id=$_GET['id'];
        try {
                $stmt = $conexao->prepare("SELECT i.usuarioid, t.categoria FROM inventario i LEFT JOIN table_itens t ON i.itemid=t.id WHERE i.id=?");
                $stmt->execute([$id]);
                $dbi = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
                $dbi = false;
        }
        if(!$dbi || $dbi['usuarioid']<>$db['id']){ echo "<script>self.location='?p=home'</script>"; exit(); }
        $categoria=$dbi['categoria'];
        $act=$_GET['action'];
        vn($id);
        try {
                $stmt = $conexao->prepare("UPDATE inventario SET status='off' WHERE usuarioid=? AND itemid IN (SELECT id FROM table_itens WHERE categoria=?)");
                $stmt->execute([$db['id'], $categoria]);
                
                if($act=='on') {
                        $stmt = $conexao->prepare("UPDATE inventario SET status='on' WHERE id=?");
                        $stmt->execute([$id]);
                }
        } catch (PDOException $e) {
                // Handle error
        }
        if($act=='on') echo "<script>self.location='?p=inventory&msg=2'</script>"; else echo "<script>self.location='?p=inventory&msg=3'</script>";
}

if(isset($_GET['sell'])){
        $id=$_GET['sell'];
        try {
                $stmt = $conexao->prepare("SELECT itemid, usuarioid FROM inventario WHERE id=?");
                $stmt->execute([$id]);
                $dbi = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
                $dbi = false;
        }
        if(!$dbi || $dbi['usuarioid']<>$db['id']){ echo "<script>self.location='?p=home'</script>"; exit(); }
        $data=date('Y-m-d H:i:s');
        try {
                $stmt = $conexao->prepare("SELECT valor FROM table_itens WHERE id=?");
                $stmt->execute([$dbi['itemid']]);
                $dbv = $stmt->fetch(PDO::FETCH_ASSOC);
                
                $stmt = $conexao->prepare("DELETE FROM inventario WHERE id=?");
                $stmt->execute([$id]);
                $affected_rows = $stmt->rowCount();
                
                if($affected_rows==0){ echo "<script>self.location='?p=home'</script>"; exit(); }
                
                $stmt = $conexao->prepare("UPDATE usuarios SET yens=yens+? WHERE id=?");
                $stmt->execute([($dbv['valor']/2), $db['id']]);
        } catch (PDOException $e) {
                echo "<script>self.location='?p=home'</script>"; exit();
        }
        echo "<script>self.location='?p=inventory&msg=4&value=".($dbv['valor']/2)."'</script>";
}

if(isset($_POST['ram_id'])){
        $id=$c->decode($_POST['ram_id'],$chaveuniversal);
        $tipo=$c->decode($_POST['ram_tipo'],$chaveuniversal);
        vn($id); vn($tipo);
        try {
                $stmt = $conexao->prepare("SELECT count(id) conta FROM ramen WHERE usuarioid=? AND id=?");
                $stmt->execute([$db['id'], $id]);
                $dbr = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
                $dbr = false;
        }
        if($dbr && $dbr['conta']>0){
                $energia=$db['energia'];
                switch($tipo){
                        case 1: $hp=50; break;
                        case 2: $hp=100; break;
                        case 3: $hp=250; break;
                        case 4: $hp=500; break;
                        case 5: $hp=1000; break;
                }
                if($energia+$hp>=$db['energiamax']) $energia=$db['energiamax']; else $energia=$energia+$hp;
                try {
                        $stmt = $conexao->prepare("DELETE FROM ramen WHERE id=?");
                        $stmt->execute([$id]);
                        
                        $stmt = $conexao->prepare("UPDATE usuarios SET energia=? WHERE id=?");
                        $stmt->execute([$energia, $db['id']]);
                } catch (PDOException $e) {
                        // Handle error
                }
                echo "<script>self.location='?p=inventory&msg=1&e=".$hp."'</script>"; exit();
        }
}

try {
        $stmt = $conexao->prepare("SELECT * FROM ramen WHERE usuarioid=?");
        $stmt->execute([$db['id']]);
        $sqlr = $stmt;
        $dbr = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
        $sqlr = false;
        $dbr = false;
}

try {
        $stmt = $conexao->prepare("SELECT i.id,i.status,i.upgrade,i.comprado_de_id,i.comprado_de_nome,i.comprado_valor,i.comprado_moeda,i.comprado_data,t.categoria,t.descricao,t.taijutsu,t.ninjutsu,t.genjutsu,t.nome,t.imagem,t.valor FROM inventario i LEFT OUTER JOIN table_itens t ON i.itemid=t.id WHERE i.usuarioid=? AND venda='nao' ORDER BY status ASC");
        $stmt->execute([$db['id']]);
        $sqli = $stmt;
        $dbi = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
        $sqli = false;
        $dbi = false;
}

// Labels e ícones de moeda para o tag "comprado de X por Y"
$inv_moeda_labels = ['yens'=>'Yens','cristal1'=>'Cristal Refinado','cristal2'=>'Cristal Bruto','cristal3'=>'Chakra Forjado'];
$inv_moeda_icons  = ['yens'=>'_img/yens.png','cristal1'=>'_img/ferreiro/Cristal de Chakra Refinado.png','cristal2'=>'_img/ferreiro/Cristal de Chakra Bruto.png','cristal3'=>'_img/ferreiro/Chakra Forjado.png'];
?>
<div class="box_top">Meu Inventário</div>
<div class="box_middle">Abaixo está todo seu inventário. Estarão listados aqui todos os itens que existirem em sua conta, incluindo os equipamentos equipados e não-equipados. Lembre-se que ao equipar um certo tipo de item, qualquer outro que seja do mesmo tipo será automaticamente substituído. Você pode também vender o item para o comércio de sua vila, ou então anunciar em sua <a href="?p=myshop">própria loja</a>, para que outros jogadores o comprem.<div class="sep"></div>
        <div align="center">
            <a href="?p=inventory"><b>Itens</b></a> |
            <a href="?p=parchments">Cristais de Refinamento</a> |
            <a href="?p=cristais_buff">Cristais de Buff</a>
        </div>
    <div class="sep"></div>
        <div style="background:url(_img/gradient.jpg) repeat-y;color:#FFFFAA;"><img src="_img/yens.png" width="14" height="14" /> <b>Meus Yens: <?php echo number_format($db['yens'],2,',','.'); ?> yens</b></div>
        <?php
        if(isset($_GET['msg'])){
                switch($_GET['msg']){
                        case 1: $msg='Ramen utilizado com sucesso!<br />Sua energia foi regenerada em <b>'.$_GET['e'].' pontos.</b>'; break;
                        case 2: $msg='Item equipado com sucesso!'; break;
                        case 3: $msg='Item desequipado com sucesso!'; break;
                        case 4: if(!isset($_GET['value'])) $value=0; else $value=$_GET['value']; $msg='Item vendido com sucesso! Foram creditados '.number_format($value,2,',','.').' yens em sua conta.'; break;
                }
        echo '<div class="sep"></div><div class="aviso">'.$msg.'</div>';
        }
        ?>
    <table width="100%" cellpadding="0" cellspacing="1">
    <?php if($dbr) { 
                // Reset to beginning for loop
                $sqlr->execute([$db['id']]);
                while($dbr = $sqlr->fetch(PDO::FETCH_ASSOC)) {
        switch($dbr['ramenid']){
                case 1: $nome='Gohan'; $reg=50; break;
                case 2: $nome='Sushi'; $reg=100; break;
                case 3: $nome='Porção de Peixe Empanado'; $reg=250; break;
                case 4: $nome='Porção de Sashimi'; $reg=500; break;
                case 5: $nome='Ramen'; $reg='1.000'; break;
        }
        ?>
    <tr>
        <td colspan="3"><div class="sep"></div></td>
    </tr>
    <tr class="table_dados" style="background:#323232;" onmouseover="style.background='#2C2C2C'" onmouseout="style.background='#323232'">
        <td width="140"><img src="_img/ramen/ramen<?php echo $dbr['ramenid']; ?>.jpg" /></td>
        <td><b><?php echo $nome; ?></b><br /><span class="sub2">Regenera <?php echo $reg; ?> pontos de Energia</span>
        <form method="post" action="?p=inventory" onsubmit="subm.value='Carregando...';subm.disabled=true;">
        <input type="hidden" id="ram_id" name="ram_id" value="<?php echo $c->encode($dbr['id'],$chaveuniversal); ?>" />
        <input type="hidden" id="ram_tipo" name="ram_tipo" value="<?php echo $c->encode($dbr['ramenid'],$chaveuniversal); ?>" />
        <input type="submit" id="subm" name="subm" class="botao" value="Usar" />
        </form>
        </td>
    </tr>
    <?php } } ?>
    <?php if($dbi) { 
                // Reset to beginning for loop
                $sqli->execute([$db['id']]);
                while($dbi = $sqli->fetch(PDO::FETCH_ASSOC)) { ?>
    <tr>
        <td colspan="2"><div class="sep"></div></td>
    </tr>
    <tr class="table_dados" style="background:#323232;" onmouseover="style.background='#2C2C2C'" onmouseout="style.background='#323232'">
        <td align="center" width="140" style="position:relative;">
          <img src="_img/equipamentos/<?php echo $dbi['imagem']; ?>.png" />
          <?php if(!empty($dbi['comprado_de_id']) && !empty($dbi['comprado_de_nome'])):
                $cm    = $dbi['comprado_moeda'] ?: 'yens';
                $cmlbl = $inv_moeda_labels[$cm] ?? 'Yens';
                $cmico = $inv_moeda_icons[$cm]  ?? '_img/yens.png';
                $cval  = (float)$dbi['comprado_valor'];
                // Yens podem ter casas decimais; cristais são sempre inteiros
                $cval_fmt = ($cm === 'yens') ? number_format($cval,2,',','.') : number_format($cval,0,',','.');
          ?>
          <div style="margin-top:4px;background:#1a1a1a;border:1px solid #ff8c1a;border-radius:4px;padding:4px 6px;font-size:10px;color:#ffd089;line-height:1.3;text-align:left;">
            <span style="color:#aaa;">Comprado de:</span><br>
            <b style="color:#ff8c1a;"><?php echo htmlspecialchars($dbi['comprado_de_nome']); ?></b><br>
            <span style="color:#aaa;">Por:</span> <b><?php echo $cval_fmt; ?></b>
            <img src="<?php echo htmlspecialchars($cmico); ?>" width="14" height="14" align="absmiddle" style="margin-left:2px;" title="<?php echo htmlspecialchars($cmlbl); ?>" alt="<?php echo htmlspecialchars($cmlbl); ?>" />
          </div>
          <?php endif; ?>
        </td>
        <td style="padding:5px;">
                <b><?php echo $dbi['nome']; ?><?php if($dbi['upgrade']>0) echo ' +'.$dbi['upgrade']; ?></b><br />
            <span class="sub2"><?php echo $dbi['descricao']; ?></span><br />
            <b><?php if($dbi['taijutsu']>0) echo '<img src="_img/equipamentos/up.png" width="14" height="14" align="absmiddle" /> [+'.($dbi['taijutsu']+$dbi['upgrade']).'] em Taijutsu<br />'; ?>
            <?php if($dbi['ninjutsu']>0) echo '<img src="_img/equipamentos/up.png" width="14" height="14" align="absmiddle" /> [+'.($dbi['ninjutsu']+$dbi['upgrade']).'] em Ninjutsu<br />'; ?>
            <?php if($dbi['genjutsu']>0) echo '<img src="_img/equipamentos/up.png" width="14" height="14" align="absmiddle" /> [+'.($dbi['genjutsu']+$dbi['upgrade']).'] em Genjutsu<br />'; ?></b>
            <br />
            <b>Valor de Venda</b>: <?php echo number_format(($dbi['valor']/2),2,',','.'); ?> yens<br />
          <a href="?p=inventory&action=<?php if($dbi['status']=='off') echo 'on'; else echo 'off'; ?>&id=<?php echo $dbi['id']; ?>"><?php if($dbi['status']=='off') echo 'Equipar'; else echo 'Retirar'; ?></a> | <a href="?p=inventory&sell=<?php echo $dbi['id']; ?>">Vender</a> | <a href="?p=addmy&id=<?php echo $dbi['id']; ?>">Anunciar em Minha Loja</a><?php if($dbi['upgrade']<10){ ?> | <a href="?p=blacksmith&id=<?php echo $dbi['id']; ?>">Aprimorar</a><?php } ?>
          </td>
        </tr>
    <?php } } ?>
    </table>
    <?php if(!$dbr && !$dbi){ ?>
    <div class="sep"></div>
    <div class="aviso">Nenhum item em seu inventário.</div>
    <?php } ?>
</div>
<div class="box_bottom"></div>

<?php
// Cristais de Buff agora estão em ?p=cristais_buff
?>
