...
if(isset($_GET['act'])){
	switch($_GET['act']){
		case 'rebel':
			...
			try {
				$upd = $conexao->prepare("UPDATE usuarios SET renegado='sim' WHERE id=?");
				$upd->execute([(int)$db['id']]);

				$texto_atu = '<a href=?p=view&view='.strtolower($db['usuario']).'>'.$db['usuario'].'</a> se tornou um membro da <b>Akatsuki</b>.';
				$ins = $conexao->prepare("INSERT INTO atualizacoes (usuarioid, texto, hora) VALUES (?, ?, ?)");
				$ins->execute([(int)$db['id'], $texto_atu, date('Y-m-d H:i:s')]);

				$del = $conexao->prepare("DELETE FROM membros WHERE usuarioid=?");
				$del->execute([(int)$db['id']]);

				// Atualiza estado em memória para que a tela seguinte já reflita o renegado.
				$db['renegado'] = 'sim';
			} catch (PDOException $e) {
				error_log('Erro ao virar Akatsuki: '.$e->getMessage());
			}
