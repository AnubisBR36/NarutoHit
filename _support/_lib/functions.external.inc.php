...
function listen($_user)
{
	...
	if(!IS_FILTERED)
	{
		if(isnull($USER->Browsers[0]->Chat))
		{
			...
			getInternal();
			$chatId = getChatId($USER->Browsers[0]->DesiredChatGroup);
...
{
	if($_GET['sv']==1) require_once('../_inc/conexao.php');
	if($_GET['sv']==2) require_once('../servidor02/_inc/conexao.php');
	
	$sqlu=mysql_query("SELECT usuario, email, vip FROM usuarios WHERE id=".($_GET['id']-50000));
	$dbu=mysql_fetch_assoc($sqlu);
	
	if(date('Y-m-d H:i:s')<$dbu['vip']) $vip=' (VIP)'; else $vip='';
	
	$data = (!isset($_GET[GET_EXTERN_USER_EMAIL])) ? getCookieValue("login_email") : base64UrlDecode($_GET[GET_EXTERN_USER_EMAIL]);
	$_html = str_replace("<!--login_value_email-->",/*htmlentities($data,ENT_QUOTES,"UTF-8")*/$dbu['email'],$_html);
	$data = (!isset($_GET[GET_EXTERN_USER_NAME])) ? getCookieValue("login_name") : base64UrlDecode($_GET[GET_EXTERN_USER_NAME]);
	$_html = str_replace("<!--login_value_name-->",/*htmlentities($data,ENT_QUOTES,"UTF-8")*/$dbu['usuario'].$vip,$_html);
...
function getChatId($_targetGroup)
{
