...
<?php
if(!isset($_GET['id']) || !is_numeric($_GET['id'])){ echo "<script>self.location='?p=home'</script>"; break; }
$stmtp = $conexao->prepare("SELECT * FROM enquetes WHERE id = ?");
$stmtp->execute([(int)$_GET['id']]);
$dbp = $stmtp->fetch(PDO::FETCH_ASSOC);
if(!$dbp){ echo "<script>self.location='?p=home'</script>"; break; }
